...
//Takes in HH:MM A(P)M
timeStrinToDateString=function(input){
  ...
  var output=new Date();
  output.setUTCHours(h);
  output.setUTCMinutes(m);
  output.setUTCSeconds(0);
  console.log('T2D Got :',input,' Output :',output.toUTCString());
  return output.toUTCString();
}
dateStringToTimeString=function(input){
  var date=new Date(input);
  var h=date.getUTCHours()
  var m=date.getUTCMinutes()
  output=String(h)+':'+String(m);
  console.log(h,m);
  console.log('D2T Got :',input,' Output :',output);
  return output;
}
//String to time converter
